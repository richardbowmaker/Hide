...
updateEditMenus :: SS.Session -> SS.TextWindow -> IO ()
updateEditMenus ss tw = do
        ...
        where   setm :: SS.Session -> SS.TextWindow -> Int -> IO ()
                setm ss tw mid = setm' ss mid (SS.twGetMenuEnabled tw mid) (SS.twGetMenuFunction tw mid)
